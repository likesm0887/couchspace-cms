...
const RelaxBanner = () => {
  ...
  const normalizeCommonData = useCallback(
    (commonData) => {
      const normalizedBannerData = Array.isArray(commonData?.[bannerKey])
        ? commonData[bannerKey]
        : [];
      return {
        ...commonData,
        [bannerKey]: normalizedBannerData,
      };
    },
    [bannerKey]
  );
  ...
  const onBannerDelete = useCallback(async (e) => {
    const currentBanners = Array.isArray(oriCommonData?.[bannerKey])
      ? oriCommonData[bannerKey]
      : [];
    if (!currentBanners.length) return;

    let select = currentBanners.findIndex(
    ...
    updatedCommonData[bannerKey].splice(select, 1);

    // 重新排序
    updatedCommonData[bannerKey].forEach((item, index) => {
      item.Seq = index + 1;
    });
    ...
    try {
      ...
      const normalizedCommonData = normalizeCommonData(commonData);
      setOriCommonData(normalizedCommonData);
      ...
      // 高效處理banner數據
      const banner = normalizedCommonData[bannerKey]?.map((e) => {
        let linkSourceName = '';
        if (e.Type === "SERIES") {
      ...
      try {
        ...
        setOriCommonData(normalizedCommonData);
        // 簡單重新加載，省略詳細的數據處理
        setDataSource(normalizedCommonData[bannerKey]?.map((e, index) => ({
          key: e.Seq,
          imageUrl: e.ImageUrl,
  ...
  }, [oriCommonData, normalizeCommonData]);
  ...
  const getData = useCallback(async () => {
    ...
    setOriCommonData(normalizedCommonData);
    console.log(normalizedCommonData[bannerKey]?.map((b) => b.LinkSourceID));
    ...
    console.log(normalizedCommonData);
    setDataSource(banner);
  }, [normalizeCommonData]);
  ...
  const onDragEnd = useCallback(async ({ active, over }) => {
    ...
    const updatedBanners = updatedDataSource.map((item) => {
      const originalItem = currentBanners.find(
  ...
  }, [oriCommonData, dataSource, normalizeCommonData]);
  ...
  const onFinish = useCallback(async () => {
    ...
    const currentBanners = Array.isArray(oriCommonData?.[bannerKey])
      ? oriCommonData[bannerKey]
      : [];
    if (!image || !linkSourceID || !selectedType) {
      return;
    }

    // 創建深拷貝避免直接修改狀態
    const updatedCommonData = JSON.parse(JSON.stringify(oriCommonData || {}));
    updatedCommonData[bannerKey] = [...currentBanners];
    updatedCommonData[bannerKey].push({
      Seq: updatedCommonData[bannerKey].length + 1,
      Type: selectedType,
      ImageUrl: image,
  ...
  }, [oriCommonData, selectedType, normalizeCommonData]);
  ...
  return (
    <div className="relax-banner">
      ...
      <Drawer
        ...
        cancelText="取消"
        okText="確定"
        bodyStyle={{ paddingBottom: 50, minHeight: 300 }}
        rootClassName="relax-banner-drawer"
        zIndex={4000}
      >
