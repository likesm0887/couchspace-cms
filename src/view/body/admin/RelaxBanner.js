...
const RelaxBanner = () => {
  ...
  const processData = useCallback(async (bannerList) => {
    const normalized = Array.isArray(bannerList) ? bannerList : [];
  ...
  const getData = useCallback(async () => {
    setLoading2(true);
    try {
      const bannerList = await meditationService.getBannersByKey(bannerKey);
  ...
  const onBannerDelete = useCallback(async (e) => {
    ...
    setLoading2(true);
    try {
      await meditationService.updateBannersByKey(bannerKey, updated);
  ...
  const onDragEnd = useCallback(async ({ active, over }) => {
    ...
    setLoading2(true);
    try { await meditationService.updateBannersByKey(bannerKey, updatedBanners); }
    catch (e) {
      console.error(e);
