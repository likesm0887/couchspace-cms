...
function Login() {
  ...
  const onClickSendForgetPasswordRequest = () => {
    ...
    counselorService
      .requestForgetPassword(new ForgetPassword(email, account))
      .then((res) => {
        console.log("res", res);
  ...
  const onClickResetPassword = () => {
    ...
    counselorService
      .resetPassword(new ResetPassword(account, verifyCode, password))
      .then((res) => {
        if (!res.success) {
          showToast(toastType.error, res.message);
        } else {
  ...
  const renderLogin = () => {
    return (
      <div
        className={"R-row"}
        style={{ paddingTop: screenWidth > 500 ? 200 : 55 }}
      >
        <div className={"C-col"} style={{ marginBottom: 0 }}>
          ...
          <input
            ...
            placeholder="信箱"
            type={"text"}
            onChange={(text) => setAccount(text.target.value.trim())}
            value={account}
          ></input>
        </div>
        ...
        <div className={"C-col"}>
          <span>
            <button className={"register-text"} onClick={() => onClickSignup()}>
              申請帳號
            </button>
          </span>
    ...
    return (
      <div className={"R-row"} style={{ paddingTop: 55 }}>
        <div className={"C-col"} style={{ marginBottom: 0 }}>
          ...
          <select
            class={identity === "" ? "select-option" : "input-identity"}
            placeholder="請選擇身份"
            onChange={(text) => setIdentity(text.target.value.trim())}
            value={identity}
          >
        ...
        <div className={"C-col"} style={{ marginBottom: 29, marginTop: 12 }}>
          <span style={{ fontSize: 16, zIndex: 998, textAlign: "center" }}>
            {"※ 如需註冊雙重身份，每個身份需使用不同信箱註冊"}
          </span>
        </div>
        <div className={"C-col"}>
          <img className="account-icon" src={img_email} alt="123"></img>
        ...
        <div className={"C-col"} style={{ marginBottom: 86 }}>
          ...
          <input
            ...
            placeholder="再次輸入密碼"
            type={"password"}
            onChange={(text) => setConfirmedPassword(text.target.value.trim())}
            value={confirmedPassword}
          ></input>
        </div>
        <div className={"C-col"}>
          <span>
            <button className={"back-text"} onClick={() => onClickBack()}>
              返回
            </button>
          </span>
          <span className={"login-wrap"}>
            <button className={"login"} onClick={() => onClickRegister()}>
              註冊
            </button>
          </span>
        </div>
      ...
      >
        ...
        <div className={"C-col"}>
          <span className={"send-wrap"}>
            <button
              className={"send"}
              style={{ color: timeLeft > 0 ? "ButtonShadow" : null }}
              disabled={timeLeft > 0}
              onClick={() => onClickSendForgetPasswordRequest()}
            >
              {" "}
              {timeLeft > 0 ? `取得驗證碼(${timeLeft}秒)` : "取得驗證碼"}
            </button>
          </span>
        </div>
        <div>
          <div className={"C-col"}>
            ...
            <input
              ...
              type={"text"}
              maxLength={6}
              onChange={(text) => setVerifyCode(text.target.value.trim())}
              value={verifyCode}
            ></input>
          </div>
          <div className={"C-col"}>
  ...
  const ErrorDialog = () => {
    return (
      <Dialog open={open} fullWidth={true} onClose={handleClose} value={"sm"}>
        <DialogTitle
  ...
  return (
    <div
      ...
      style={{ height: 1024, backgroundColor: "#f7f8f8", paddingLeft: 0 }}
    >
      {screenWidth > 500 ? (
        <div className={"content"}>
          <div class="row">
            <img
              style={{ height: 29, width: "100%", marginTop: 87 }}
              src={img_logo}
              alt="123"
            />
          </div>
        ...
        <div
          class="col-sm-12 h-100 align-items-center"
          style={{ justifyItems: "center" }}
        >
          <div class="row">
            <img
              style={{ height: 29, width: 240, marginTop: 47 }}
              src={img_logo}
              alt="123"
            />
          </div>
